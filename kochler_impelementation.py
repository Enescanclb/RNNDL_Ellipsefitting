...
for k in range(200):
    ...
    start_angle=int(np.arctan2(current_meas[0][1],current_meas[0][0])*180/np.pi)
    stop_angle=int(np.arctan2(current_meas[-1][1],current_meas[-1][0])*180/np.pi)
    angles=np.arange(start_angle-25,stop_angle+25,delta_a)
    ...
    for numz in range(nk):
        diff=np.array(np.transpose(current_meas[numz])-zk)
        Zk += diff.T @ diff
    ...
    P_kk=P_kkm-(W_kkm @ W_kkm.T * S_kkm)
    N_kkm=(zk.T-np.kron(H,Identity_matrix)@x_kkm)@np.transpose((zk.T-np.kron(H,Identity_matrix)@x_kkm))*(S_kkm**(-1))
    X_kk=X_kkm+N_kkm+Zk
    v_kk=v_kkm+nk
    ...
    for i in range(I_repeats):
        ...
        for a in angles:
            intersection_point=intersection_points(ellipse_params,a)
            if not len(intersection_point):
                virtual_measurements=np.delete(virtual_measurements,b,0)
            elif any(np.isnan(intersection_point)):
                virtual_measurements=np.delete(virtual_measurements,b,0)
            else:
        ...
        E_Y_K = Y_tilda_ki/vnk
        eigvalues_y,eigvectors_y=np.linalg.eig(E_Y_K)
        ...
        u_kip=u_ki+(K_constant*(error_i))
        if any(t < 0 for t in u_kip):
            u_kip = np.absolute(u_kip)
...
def update(frame):
    ...
    title='Output Ellipse' + str(index)
    plt.title(title)
    ax.legend()
